...
char * GPU_FAILURERJ__Rep1__SharedNames[] = {"FLOAT_ALU_FAILURE", "INSTRUCTION_READY", "INT_ALU_FAILURE", "KO_CONTENT", "MEM_FAILURE", "OK_CONTENT", "REGISTERS_FILL", "REG_FAILURE", "RESULT_KO", "RESULT_OK", "SCHEDULER", "WARP_ACCESS_DRAM", "WARP_ACCESS_L1", "WARP_ACCESS_L2"};

GPU_FAILURERJ__Rep1::GPU_FAILURERJ__Rep1():Rep("Rep1", nwarps, 14, GPU_FAILURERJ__Rep1__SharedNames)
{
  InstanceArray = new GPU_FAILURERJ__WARP_REG * [NumModels];
  delete[] ModelArray;
  ModelArray = (BaseModelClass **) InstanceArray;
  for (counter = 0; counter < NumModels; counter++)
    InstanceArray[counter] = new GPU_FAILURERJ__WARP_REG();
...
GPU_FAILURERJ__Rep1::~GPU_FAILURERJ__Rep1() {
  if (NumModels == 0) return;
