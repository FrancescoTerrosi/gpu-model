...
char * GPU_FAILURERJ__PROCESSOR_REP__SharedNames[] = {"INSTRUCTION_READY", "MEM_FAILURE", "READ_DRAM", "READ_L1", "READ_L2", "WARP_ACCESS_DRAM", "WARP_ACCESS_L1", "WARP_ACCESS_L2", "WRITE_DRAM", "WRITE_L1", "WRITE_L2"};

GPU_FAILURERJ__PROCESSOR_REP::GPU_FAILURERJ__PROCESSOR_REP():Rep("PROCESSOR_REP", nprocessors, 11, GPU_FAILURERJ__PROCESSOR_REP__SharedNames)
{
  InstanceArray = new GPU_FAILURERJ__PROCESSOR * [NumModels];
  delete[] ModelArray;
  ModelArray = (BaseModelClass **) InstanceArray;
  for (counter = 0; counter < NumModels; counter++)
    InstanceArray[counter] = new GPU_FAILURERJ__PROCESSOR();
...
GPU_FAILURERJ__PROCESSOR_REP::~GPU_FAILURERJ__PROCESSOR_REP() {
  if (NumModels == 0) return;
