...
char * GPU4RJ__PROCESSOR_REP__SharedNames[] = {"KO_READ", "MEM_OP_COMPLETE", "OK_READ", "READ_DRAM", "READ_L1", "READ_L2", "RESULT_KO", "RESULT_OK", "WRITE_DRAM", "WRITE_L1", "WRITE_L2"};

GPU4RJ__PROCESSOR_REP::GPU4RJ__PROCESSOR_REP():Rep("PROCESSOR_REP", nprocessors, 11, GPU4RJ__PROCESSOR_REP__SharedNames)
{
  InstanceArray = new GPU4RJ__PROCESSOR * [NumModels];
  delete[] ModelArray;
  ModelArray = (BaseModelClass **) InstanceArray;
  for (counter = 0; counter < NumModels; counter++)
    InstanceArray[counter] = new GPU4RJ__PROCESSOR();
...
GPU4RJ__PROCESSOR_REP::~GPU4RJ__PROCESSOR_REP() {
  if (NumModels == 0) return;
  delete KO_READ;
