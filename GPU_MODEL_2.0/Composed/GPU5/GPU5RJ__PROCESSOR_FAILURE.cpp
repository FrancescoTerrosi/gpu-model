...
char * GPU5RJ__PROCESSOR_FAILURE__SharedNames[] = {"FLOAT_ALU_FAILURE", "INSTRUCTION_READY", "INT_ALU_FAILURE", "KO_CONTENT", "MEM_FAILURE", "OK_CONTENT", "READ", "READ_DRAM", "READ_L1", "READ_L2", "REGISTERS_FILL", "RESULT_KO", "RESULT_OK", "SCHEDULER", "WARP_ACCESS_DRAM", "WARP_ACCESS_L1", "WARP_ACCESS_L2", "WRITE", "WRITE_DRAM", "WRITE_L1", "WRITE_L2"};

GPU5RJ__PROCESSOR_FAILURE::GPU5RJ__PROCESSOR_FAILURE():Join("PROCESSOR_FAILURE", 3, 21,GPU5RJ__PROCESSOR_FAILURE__SharedNames) {
  EXEC_UNIT_FAILURE = new EXEC_UNITSAN();
  ModelArray[0] = (BaseModelClass*) EXEC_UNIT_FAILURE;
  ModelArray[0]->DefineName("EXEC_UNIT_FAILURE");
  MEMORY_FAILURE = new MEMORYSAN();
  ModelArray[1] = (BaseModelClass*) MEMORY_FAILURE;
  ModelArray[1]->DefineName("MEMORY_FAILURE");
  Rep2 = new GPU5RJ__Rep2();
  ModelArray[2] = (BaseModelClass*) Rep2;
  ModelArray[2]->DefineName("Rep2");
...
GPU5RJ__PROCESSOR_FAILURE::~GPU5RJ__PROCESSOR_FAILURE() {
  if (!AllChildrenEmpty()) {
